...
## 一枚絵（フラット化された図解）の再構築 — v2.6.3（実験的）
...
4. **OCR テキスト化** — オフライン **PP-OCRv5**（`onnxocr` / Apache-2.0、
   ...
   検出は原寸のまま行い、認識に渡す行クロップだけを 3 倍拡大するため、
   省メモリなまま白抜き文字・小さい日本語もそのまま読める。
   認識した行は編集可能なテキストボックスに置き換える。
   誤読テキストを出さないよう、安全ゲートを通ったものだけを採用する:
   - **確信度ゲート**: 確信度の低い行は不採用（背景板上はさらに厳しく）
   - **再描画照合**: 認識したテキストをフォントで実際に描画し、元の字形と
     縦横比・インク分布を比較。形が合わない認識結果を棄却する。
     照合は CV 分解の結果ではなく**作業画像そのもの**に対して行うため、
     行が複数パーツに割れていても正しく照合できる。CJK 主体の行は
     縦横比の下限を厳しくして「文字の読み落とし」も弾く。
     罫線・枠線の行はプロファイルから除外し、白抜き行（帯の上の白文字）は
     帯色を下地にして照合する
   - **行頭アイコンの切り離し**: 「アイコン＋テキスト」が 1 行として
     検出された場合、先頭のインク群を最初の大きな空白で切り離して
     再照合する（丸バッジもこの一般化された処理でまとめて扱う）
   - **字形の混同補正**: カタカナ語中の「夕→タ」「二→ニ」「口→ロ」等、
     字形が酷似する漢字への誤認識を文脈判定で補正する
   - 商品写真の中に写り込んだ文字（パッケージのロゴ等）は照合で自然に
     弾かれ、写真のまま残る
   ゲートを通らない行は画像のまま残す
