...
4. **OCR テキスト化** — オフライン **PP-OCRv5**（`onnxocr` / Apache-2.0、
   モデル同梱）で作業画像を検出+認識する（`ocr_engine.py` + `decompose.js`）。
   検出は原寸、認識は行クロップだけを拡大する省メモリ設計。
   検出枠は行末の文字が欠けないよう行高に応じて広げてから認識する。
   認識した行は編集可能なテキストボックスに置き換える。
   誤読テキストを出さないよう、安全ゲートを通ったものだけを採用する:
   - **2 スケール一致**: 同じ行を 3 倍と 1.5 倍の 2 スケールで認識し、
     読みが一致しない行は誤読の疑いが濃いとして原則不採用にする
     （劣化画像の誤読はスケールに敏感で、正しい読みは安定している）
   - **確信度ゲート**: 確信度の低い行は不採用（背景板上はさらに厳しく）
   - **2D 形状照合**: 認識テキストを描画し、元のインク形状と 2 次元
     グリッドで照合する。しきい値は行内のコントラストに適応させ、
     圧縮でにじんだ小さな文字も安定して測れる。枠線・罫線・グラフ軸は
     連結成分解析でマスクから除去し、白抜き行は帯色を推定して反転する
   - **行頭バッジ・アイコンの切り離し**: 「そのまま」と「先頭を切り離した
     解釈」の両方を照合し、良い方を採用する（バッジは画像のまま残す）
   - **字形の混同補正**: 夕→タ・二→ニ等の字形類似漢字、拗音の大書き
     （じや→じゃ）、簡体字（问→問 等）を文脈判定で補正する
   - **写真テキスト保護**: 商品写真・パッケージの中の文字（ロゴ等）は
     下地のテクスチャと色で検出してテキスト化せず、写真を壊さない
   ゲートを通らない行は画像のまま残す
   （「読めた風の間違ったテキスト」より「正確な画像」を優先）
5. **再構築** — スライドを「きれいな背景板 + 各パーツ（透過 PNG）+
   テキストボックス」で組み直す。テキストボックスは必ず全パーツより
   前面（Z順の最上位）に配置するため、見た目が文字の位置でもクリックが
   画像に吸われず、そのまま打ち替え編集できる。各パーツも個別に
   移動・削除・差し替えができる。消し込みは行のインク画素だけを対象に
   するため、行間が詰まった図解でも隣の行を巻き込まない。最後に
   **整合復元**として、出力パーツの合成が元画像と一致しない画素を
   復元し（意図的に消したテキスト領域を除く）、見た目の欠けを防ぐ

長体（横に潰した文字）はフォントサイズを幅に合わせて調整します。
分解の判定や解析に失敗した場合は、必ず従来どおりの変換結果をそのまま
返します。PP-OCRv5（python3 + onnxocr）が無い環境では tesseract
(jpn+eng) に自動フォールバックし、それも無ければ OCR だけがスキップ
されます（分解のみ実施）。
...
**必要なもの:** Node.js、LibreOffice (impress/draw/writer)、Chromium（HTML 変換に使用）、python3 + `pip install onnxocr`（一枚絵の OCR テキスト化に使用・無くても動作）、tesseract-ocr + tesseract-ocr-jpn（onnxocr が無い場合の OCR フォールバック・無くても動作）
