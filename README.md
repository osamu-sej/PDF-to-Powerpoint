...
## 一枚絵（フラット化された図解）の再構築 — v2.6.0（実験的）
...
4. **OCR テキスト化** — オフライン **PP-OCRv5**（`onnxocr` / Apache-2.0、
   モデル同梱）で作業画像全体を 3 倍拡大して一括認識する
   （`ocr_engine.py` + `decompose.js`）。検出モデルが文字行の位置を
   正確に見つけるため、白抜き文字・小さい日本語もそのまま読める。
   ...
   - **再描画照合**: 認識したテキストをフォントで実際に描画し、元の字形と
     ...
     帯色を下地にして照合する
   - **丸バッジ検出**: 行頭の丸バッジ（❶❷ⒶⒷ…）は誤読の温床なので
     幾何判定（内接円テスト）で行から外し、バッジ自体は画像のまま残す
